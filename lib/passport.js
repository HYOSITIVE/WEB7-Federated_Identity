...
// by HYOSITIVE
// based on WEB7 - Passport_Facebook - 8
...
module.exports = function(app) { // passport.js라는 모듈은 함수 자체
    ...
    facebookCredentials.profileFields = ['id', 'emails', 'name', 'displayName']; // 이메일 획득을 위해 추가
        passport.use(new FacebookStrategy(facebookCredentials,
        function(accessToken, refreshToken, profile, done) {
            console.log('FacebookStrategy', accessToken, refreshToken, profile);
            var email = profile.emails[0].value; // 페이스북 API로부터 받아 온 사용자 이메일
            var user = db.get('users').find({email:email}).value(); // 기존 사용자 중 같은 이메일 존재하는지 탐색
            if (user) { // 사용자 존재
                user.facebookId = profile.id; // 사용자 정보에 페이스북 ID 추가
                db.get('users').find({id:user.id}).assign(user).write(); // DB에 저장
            }
            else { // 사용자 미존재
                user = {
